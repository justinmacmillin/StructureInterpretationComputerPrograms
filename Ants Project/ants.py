...
Name:Erik Bartlett, Justin MacMillin
Login:cs61a-aie, cs61a-aif
TA:Hamilton Nguyen
Section:12
"""
        ...
        if not insect.is_ant():
            ...
            if insect.container and insect.ant:
                ...
                if insect.name == 'Queen':
                    if insect.num == 0:
                        print("You can't remove the queen ant!")
                    else:
                        self.ant = None
                        insect.place = None
                else:
...
class QueenPlace(object):
    """Place that holds a Queen Ant"""
    
    def __init__(self, place, colony):
        self.colony = colony
        self.place = place
    @property
    def bees(self):
        return self.place.bees + self.colony.bees
...
class Ant(Insect):
    ...
    container = False
    blocks_path = True
    damage_inc = False # For the implementation of the Queen Ant's doubling effect
    ...
    def can_contain(self, ant):
        if self.container and not ant.container and self.ant == None:
            return True
        else:
            return False
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        spaces = 0
        place = self.place
        while place.name != 'Hive' and spaces <= self.min_range:
            spaces += 1
            place = place.entrance
        while place.name != 'Hive' and len(place.bees) == 0:
            spaces += 1
            place = place.entrance
        if spaces >= self.min_range and spaces <= self.max_range and place.name != 'Hive':
...
class QueenAnt(ThrowerAnt):
    ...
    def action(self, colony):
        ...
        if self.num == 0:
            ThrowerAnt.throw_at(self, self.nearest_bee(colony.hive))
            colony.queen = QueenPlace(self.place, colony.queen)
            while hasattr(place, 'exit'):
                if place.ant:
                    if not place.ant.damage_inc:
                        place.ant.damage += place.ant.damage
                        place.ant.damage_inc = True
                place = place.exit
        else:
