...
def apply_primitive(procedure, args, env):
    ...
    p = args
    while p != nil:
        new_args.append(p.first)
        p = p.second
    if procedure.use_env:
        new_args.append(env)
    try:
        return procedure.fn(*new_args)
    except TypeError:
        raise SchemeError('Invalid argument type')
...
def do_lambda_form(vals, env):
    ...
    if len(vals.second) > 1:
        vals.second = Pair(Pair('begin', vals.second), nil)
...
def do_define_form(vals, env):
    ...
    if scheme_symbolp(target):
        check_form(vals, 2, 2)
        if isinstance(vals.second.first, Pair):
            second = scheme_eval(vals.second.first, env)
            env.define(target, second)
        else:
            env.define(target, vals.second.first)
    elif isinstance(target, Pair):
        formals = vals.first.second
        body = vals.second.first
...
def do_let_form(vals, env):
    ...
    temp = bindings
    while temp != nil:
        check_form(temp.first, 1, 2)
        if not scheme_symbolp(temp.first.first):
            raise SchemeError('invalid formal parameters')
        names = Pair(temp.first.first, names)        
        if isinstance(temp.first.second.first, Pair):
            eval_names = scheme_eval(temp.first.second.first, env)
            vals = Pair(eval_names, vals)
        else:
            vals = Pair(temp.first.second.first, vals)
        temp = temp.second
...
def do_if_form(vals, env):
    ...
    check_form(vals, 3, 3)
    target = vals[0]
    first = vals.second.first
    second = vals.second.second.first
    if scheme_true(scheme_eval(target, env)):
        return first
    else:   
        return second
...
def do_and_form(vals, env):
    ...
    while current.second != nil:
        if not scheme_true(scheme_eval(current.first, env)):
...
def do_or_form(vals, env):
    ...
    if vals == nil:
        return False
    while current.second != nil:
        if scheme_true(scheme_eval(current.first, env)):
            return current.first
        current = current.second
    return current.first
...
def do_begin_form(vals, env):
    ...
    if vals == nil:
        ...
        looper = vals
        while looper.second != nil:
            scheme_eval(looper.first, env)
            looper = looper.second
        return looper.first
...
def check_formals(formals):
    ...
    >>> check_formals(read_line("(sum n total)"))
    """
    current = formals
    if isinstance(formals, Pair):
        while current != nil:
            if current.first in current.second:
                raise SchemeError('Invalid formal parameters')
            if current.first == '.' and current.second != nil:
                raise SchemeError('Not a well formed list of parameters')
            if not scheme_symbolp(current.first):
                raise SchemeError('Invalid define expression')
            current = current.second
    elif len(formals) == 0:
        return None
    else:
