...
def extract_words(text):
    ...
    for i in range(len(words)):
        if words[i] not in ascii_letters:
            words[i] = ' '
...
def analyze_tweet_sentiment(tweet):
    ...
    False
    """
    sentiments = [sentiment_value(get_word_sentiment(word)) for word in tweet_words(tweet) if has_sentiment(get_word_sentiment(word))]
    if len(sentiments) == 0:
...
def find_centroid(polygon):
    ...
    for i in range(len(polygon)-1):
        x1, x2 = polygon[i][0], polygon[i+1][0]
        y1, y2 = polygon[i][1], polygon[i+1][1]
        centroid_lat += (x1 + x2)*(x1*y2-x2*y1)
        centroid_long += (y1 + y2)*(x1*y2-x2*y1)
        a += (x1*y2 - x2*y1)/2
    if a == 0:
        return polygon[0][0], polygon[0][1], 0

    return (centroid_lat/(a*6)), (centroid_long/(a*6)), abs(a)
...
def find_center(polygons):
    ...
    -156.21763
    """
    centroids_long = [find_centroid(poly)[1] for poly in polygons]
    centroids_lat = [find_centroid(poly)[0] for poly in polygons]
    a = [find_centroid(poly)[2] for poly in polygons]
    c_long = 0
    c_lat = 0
    total_a = 0
    for i in range(len(polygons)):
        c_long += centroids_long[i] * a[i]
        c_lat += centroids_lat[i] * a[i]
        total_a += a[i]
    return c_lat/total_a, c_long/total_a
...
def find_closest_state(tweet, state_centers):
    ...
    'MO'
    """
    keys = [key for key in state_centers]
    centers = dict(state_centers)
    for key in keys:
        centers[key] = geo_distance(tweet_location(tweet), centers[key])
    inverse = {}
    for key, val in centers.items():
        inverse[val] = key
    secondkeys = [key for key in inverse]
    return inverse[min(secondkeys)]
...
def most_talkative_state(term):
    ...
    sorted_tweets = group_tweets_by_state(tweets)
    most_talkative = ''
    most = 0
    for state in sorted_tweets:
        if len(sorted_tweets[state]) > most:
            most_talkative = state
            most = len(sorted_tweets[state])
...
def average_sentiments(tweets_by_state):
    ...
    for state in tweets_by_state:
        ...
        for elem in tweets_by_state[state]:
            if analyze_tweet_sentiment(elem) != None:
                not_none += 1
        if not_none > 0:
            avg_sent = reduce(add, [analyze_tweet_sentiment(tweet) for tweet in tweets_by_state[state] if analyze_tweet_sentiment(tweet) != None])/len([tweet for tweet in tweets_by_state[state] if analyze_tweet_sentiment(tweet) != None])
            averaged_state_sentiments[state] = avg_sent
